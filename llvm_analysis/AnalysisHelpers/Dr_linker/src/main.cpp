...
#define SIZEOF_ARRAY(a) (sizeof(a) / sizeof(a[0])) // get the lens of a array
...
// folders
// ucore_plus folders
static const char *ucore_plus_driver_folder[] = {"kern-ucore/module/drivers"};  // there may be some problems
...
int is_interesting_folder(char *curr_folder, unsigned long arch_no) {
    ...
    switch(arch_no) {
        ...
        case 4:
            ...
            break;
        case 5:
            inter_folders = ucore_plus_driver_folder;
            array_size = SIZEOF_ARRAY(ucore_plus_driver_folder);
            break;
        default:
            std::cerr << "Invalid arch number, Valid arch numbers are: 1(mediatek)|2(qualcomm)|3(huawei)|4(samsung)|5(ucore_plus)\n";
            exit(-2);
    }
...
int main(int argc, char *argv[]) {             /// llvm_bc_out  chipset_num
    //check args
    if(argc < 3) {
        print_err(argv[0]);
    }
    ...
    arch_no = strtol(argv[2], &dummy_ptr, 10); // to long int
